...
function getBathValue() {
  ...
  return parseInt(uiBathrooms.value) || -1; // Return the selected value or -1 if no valid value is selected
}

    function getBHKValue() {
      var uiBHK = document.getElementById("uibhk");
      return parseInt(uiBHK.value) || -1; // Return the selected value or -1 if no valid value is selected
  }
    ...
    function onClickedEstimatePrice_new() {
      console.log("Estimate price button clicked");
      var sqft = document.getElementById("uiSqft");
      var bhk = getBHKValue();
      var bathrooms = getBathValue();
      ...
      $.post(url, {
          total_sqft: parseFloat(sqft.value),
          balcony_count:balcony.value,
          bhk: bhk,
          bath: bathrooms,
          area_type :area.value,
          location: location.value
      },function(data, status) {
          console.log(data.estimated_price_new);
          estPrice.innerHTML = "<h4>" + data.estimated_price.toString() + " Lakh</h4>";
          console.log(status);
      });
    }
